...
aas = function(str)
  ...
  local class = bindClass(str)
  local super = class.getSuperclass()

  super = super and res.string.parent_class .. " " .. tostring(super.getName())

  table.insert(eee.aa,super)

  for i = 0,#class.getClasses()-1
    table.insert(eee.aa,tostring(class.getClasses()[i]))
  end

  --构建方法
  for i = 0,#class.getConstructors()-1
  ...
  for n = 0,#getf-1
    ...
    if ex == "java.lang.Object"

      insert(eee.cc,str)

     else

      if not str:find("Object%.")
    ...
    if g1
      ...
      insert(eee.gg,string_format("(%s)%s",g1,g2))
...
for _,lib pairs{'_G', 'string', 'table', 'math',
...
dump = function(value, varname, fastmode, ident)
  ...
  for _,k ipairs(lua_reserved_keywords)
    keycache[k] = '["'..k..'"] = '
  end
  ...
  if fastmode
    ...
    fcts.table = function (value, ident, path)
      ...
      if test_defined(value, path) return "nil" end
      -- Table value
      ...
      for _,key pairs(keys(value))
        ...
        if key == numidx
          ...
          s = keycache[key]
          if not s:match "^%[" subpath = subpath .. "." end
      ...
      local _str = "{"..sep..table_concat(str, ","..sep).." "..sep:sub(1,-3).."}"

      if meta
        sep = sep:sub(1,-3)
        return "setmetatable("..sep.._str..","..sep..metastr..sep:sub(1,-3)..")"
      end
  ...
  dumplua = function(value, ident, path)
...
tabMerge = function(tab1,tab2)

  local tab = table.clone(tab1)

  table_foreach(tab2,function(k,v)
    insert(tab,v)
  end)
